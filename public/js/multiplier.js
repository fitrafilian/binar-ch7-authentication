...
socket.on("room-joined", (id) => {
  ...
  //   .then((response) => response.json())
  //   .then((response) => {
  //     return (roomDB = response);
  //   });

  // function getFromAPI(url) {
  //   fetch(url)
  //     .then((res) => res.json())
  //     .then((data) => (roomDB = data));
  // }

  // getFromAPI(settings.url);
...
socket.on("player-2-disconnected", async () => {
  if (myScorePoints !== 0 || enemyScorePoints !== 0) {
    const dataScore = {
      roomId: roomId,
      roomDB: roomDB,
    ...
    await fetch("/games/multiplierpost", {
      method: "POST", // or 'PUT'
      headers: {
        "Content-Type": "application/json",
      },
